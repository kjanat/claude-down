...
const WEBVIEW_WORKER_TIMEOUT_MS = WEBVIEW_NAVIGATION_TIMEOUT_MS
	+ WEBVIEW_SNAPSHOT_TIMEOUT_MS + 5000;
const CHROME_ARGV = [
	'--headless=new',
	'--disable-gpu',
	'--no-sandbox',
...
async function pollPogoSnapshot(view) {
	...
	JSON.stringify(POGO_SNAPSHOT_EXPRESSION)
}).catch(() => null);
...
async function checkDownDetectorWithWebView(
	...
	// boundary.
	return checkWithWebViewWorker(
		url,
		chromePath ?? process.env[CHROME_PATH_ENV],
	);
}
